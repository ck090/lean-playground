    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            ## Model inputs going into the next state for the Beam search algorithm
            ## This is also the same as the output of the previous step
            ################################################################################

            # Append to a tree and return that, indicating every step of the process
            
            beam_tree[str(i)] = input_ids # (level, tokens)  
            ...
            ################################################################################
            # print(next_token_scores, len(next_token_scores), next_token_scores[0].shape, next_token_scores[1].shape)
            
            beam_score_tree[str(i)] = next_token_scores #[(2 x # output tokens)]
            i += 1
        ...
        if return_dict_in_generate:
            ...
            if inference:
                return sequence_outputs["sequences"], beam_tree, beam_score_tree # return beam search results as well
            else:
